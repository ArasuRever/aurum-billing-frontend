...
function VendorDetails() {
  ...
  const getAllowedMetals = () => { if (!vendor) return []; if (!vendor.vendor_type || vendor.vendor_type === 'BOTH') return productTypes; const specific = productTypes.filter(t => t.name === vendor.vendor_type); return specific.length > 0 ? specific : productTypes; };
  const initStockForm = () => { fetchMasterItems(); const allowed = getAllowedMetals(); const defaultMetal = allowed.length > 0 ? allowed[0].name : 'GOLD'; setStockRows([{ metal_type: defaultMetal, stock_type: 'SINGLE', quantity: 1, item_name: '', huid: '', gross_weight: '', wastage_percent: '', making_charges: '', item_image: null, calc_total_pure: 0 }]); setBatchInvoice(''); setViewMode('add_stock'); setPurityMode('TOUCH'); };
  const handleAddRow = () => { const allowed = getAllowedMetals(); const defaultMetal = allowed.length > 0 ? allowed[0].name : 'GOLD'; setStockRows([...stockRows, { metal_type: defaultMetal, stock_type: 'SINGLE', quantity: 1, item_name: '', huid: '', gross_weight: '', wastage_percent: '', making_charges: '', item_image: null, calc_total_pure: 0 }]); };
  ...
  const getTxnBadge = (txn) => {
      ...
      if (txn.reference_type === 'ITEM_RESTORE' || (txn.description && txn.description.includes('Restored:'))) {
          return <span className="fw-bold text-primary">RESTORED</span>;
      }
      if(txn.type === 'STOCK_ADDED') return <span className="fw-bold text-danger">STOCK</span>;
      if(txn.type === 'STOCK_UPDATE') return <span className="fw-bold text-primary">UPDATE</span>;
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', backgroundColor:'rgba(0,0,0,0.5)', zIndex:1050, display:'flex', alignItems:'center', justifyContent:'center'}}>
            <div className="bg-white p-4 rounded shadow-lg" style={{backgroundColor:'white', padding:'20px', borderRadius:'8px', width:'450px', maxWidth:'90%'}}>
                ...
                <p className="mb-3">
                    You are about to delete <strong>{vendor.business_name}</strong>.
                ...
                <div className="d-flex justify-content-end gap-2">
                    <button onClick={() => setShowDeleteModal(false)} className="btn btn-light">Cancel</button>
          ...
          <div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
              <div className="modal-dialog">
                  <div className="modal-content">
                      ...
                      <div className="modal-body"><p className="mb-2"><strong>Item:</strong> {restockItem.item_name} ({restockItem.barcode})</p><div className="row g-2 mb-3"><div className="col-6"><label className="small">Added Weight</label><input className="form-control" type="number" value={restockForm.gross_weight} onChange={e => setRestockForm({...restockForm, gross_weight: e.target.value})} /></div><div className="col-6"><label className="small">Added Qty</label><input className="form-control" type="number" value={restockForm.quantity} onChange={e => setRestockForm({...restockForm, quantity: e.target.value})} /></div></div><div className="mb-3"><label className="small">Invoice / Ref</label><input className="form-control" value={restockForm.invoice_no} onChange={e => setRestockForm({...restockForm, invoice_no: e.target.value})} /></div><div className="mb-3"><label className="small">New Wastage % (Optional)</label><input className="form-control" type="number" value={restockForm.wastage_percent} onChange={e => setRestockForm({...restockForm, wastage_percent: e.target.value})} /></div></div>
                      <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setRestockItem(null)}>Cancel</button><button className="btn btn-success" onClick={handleRestockSubmit}>Confirm Restock</button></div>
                  </div>
              </div>
          </div>
      )}
      {historyItem && (
          <div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
              <div className="modal-dialog modal-lg">
                  <div className="modal-content">
                      <div className="modal-header"><h5 className="modal-title">Batch History: {historyItem.item_name}</h5><button className="btn-close" onClick={() => setHistoryItem(null)}></button></div>
                      <div className="modal-body p-0"><table className="table table-striped mb-0 small"><thead className="table-light"><tr><th>Date</th><th>Type</th><th>Qty Change</th><th>Wt Change</th><th>Ref</th></tr></thead><tbody>{itemHistory.map((log, i) => (<tr key={i}><td>{new Date(log.created_at).toLocaleString()}</td><td><span className={`badge ${log.quantity_change > 0 ? 'bg-success' : 'bg-danger'}`}>{log.action_type}</span></td><td className="fw-bold">{log.quantity_change > 0 ? '+' : ''}{log.quantity_change}</td><td>{log.weight_change > 0 ? '+' : ''}{log.weight_change} g</td><td>{log.related_bill_no || '-'}</td></tr>))}{itemHistory.length === 0 && <tr><td colSpan="5" className="text-center p-3">No history found</td></tr>}</tbody></table></div>
                  </div>
              </div>
          </div>
      )}
         ...
         <div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
            <div className="modal-dialog modal-lg">
               <div className="modal-content">
                  ...
                  <div className="modal-body">
                     <div className="row g-2 mb-3 bg-light p-2 rounded"><div className="col-md-4"><input className="form-control form-control-sm" placeholder="Agent Name" value={agentForm.agent_name} onChange={e => setAgentForm({...agentForm, agent_name: e.target.value})} /></div><div className="col-md-4"><input className="form-control form-control-sm" placeholder="Phone" value={agentForm.agent_phone} onChange={e => setAgentForm({...agentForm, agent_phone: e.target.value})} /></div><div className="col-md-4 d-flex"><input type="file" className="form-control form-control-sm me-2" onChange={e => setAgentForm({...agentForm, agent_photo: e.target.files[0]})} /><button className="btn btn-primary btn-sm" onClick={handleSaveAgent}>{isEditingAgent?'Update':'Add'}</button></div></div>
         ...
         <div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)'}}>
            <div className="modal-dialog">
               <div className="modal-content">
                  ...
                  <div className="modal-body"><input className="form-control mb-2" placeholder="Name" value={editVendorForm.business_name} onChange={e => setEditVendorForm({...editVendorForm, business_name: e.target.value})} /><input className="form-control mb-2" placeholder="Contact" value={editVendorForm.contact_number} onChange={e => setEditVendorForm({...editVendorForm, contact_number: e.target.value})} /><select className="form-select mb-2" value={editVendorForm.vendor_type} onChange={e => setEditVendorForm({...editVendorForm, vendor_type: e.target.value})}><option value="BOTH">All Metals (Both)</option>{productTypes.map(t => (<option key={t.id} value={t.name}>{t.name} Only</option>))}</select><textarea className="form-control mb-2" placeholder="Address" value={editVendorForm.address} onChange={e => setEditVendorForm({...editVendorForm, address: e.target.value})} /><input className="form-control mb-2" placeholder="GST" value={editVendorForm.gst_number} onChange={e => setEditVendorForm({...editVendorForm, gst_number: e.target.value})} /></div>
