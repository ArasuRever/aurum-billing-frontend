...
function ExternalGST() {
  ...
  // Items Grid (Updated with Wastage & Discount)
      ...
      const newItems = items.map(item => {
          ...
          if(field === 'item_name') {
              ...
              if(match) {
                  ...
                  // Auto-set Rate based on Metal Type
          ...
          // 3. Final Total: ((Gross + WastageWt) * Rate) + MC - Discount
  ...
  const handleSave = async () => {
      ...
      const payload = { invoice_no: invoiceNo, date: billDate, customer, items, exchangeItems, totals: { taxable: taxableTotal, cgst, sgst, exchange: exchangeTotal, final: finalTotal } };
      try {
  ...
  const handlePrint = async (bill) => {
      try {
          ...
          const data = {
              invoice_id: b.invoice_number, date: new Date(b.bill_date).toLocaleDateString(),
              customer: { name: b.customer_name, phone: b.customer_phone, address: b.customer_address, id: b.customer_gstin?`GST: ${b.customer_gstin}`:'' },
              // Map to InvoiceTemplate format (Includes Wastage/Discount)
              items: i.map(x => ({ 
                  item_name: x.item_name, gross_weight: x.gross_weight, wastage_percent: x.wastage_percent, wastage_weight: x.wastage_weight, rate: x.rate, making_charges: x.making_charges, discount: x.discount_amount, total: parseFloat(x.taxable_value) 
              })),
              exchangeItems: e ? e.map(ex => ({ name: ex.item_name, net_weight: ex.net_weight, rate: ex.rate, total: parseFloat(ex.total_amount) })) : [],
              totals: { grossTotal: parseFloat(b.gross_total), totalDiscount: 0, sgst: parseFloat(b.sgst_amount), cgst: parseFloat(b.cgst_amount), exchangeTotal: parseFloat(b.exchange_total||0), netPayable: parseFloat(b.total_amount), paidAmount: parseFloat(b.total_amount), balance: 0 },
              includeGST: true
          };
                ...
                <div className="col-md-12">
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-4">
                            <div className="card bg-white shadow-sm border-0 h-100"><div className="card-body"><div className="d-flex justify-content-between mb-1"><span>Taxable:</span><span>{taxableTotal.toFixed(2)}</span></div><div className="d-flex justify-content-between mb-1 text-muted small"><span>CGST (1.5%):</span><span>{cgst.toFixed(2)}</span></div><div className="d-flex justify-content-between mb-3 text-muted small"><span>SGST (1.5%):</span><span>{sgst.toFixed(2)}</span></div><div className="d-flex justify-content-between mb-3 pt-2 border-top fw-bold"><span>Bill Total:</span><span>₹{billTotal.toFixed(2)}</span></div>{exchangeTotal > 0 && <div className="alert alert-success p-1 px-2 mb-3 d-flex justify-content-between small"><span className="fw-bold">Less Exchange</span><span className="fw-bold">- {exchangeTotal}</span></div>}<div className="d-flex justify-content-between pt-2 border-top"><span className="fw-bold fs-4">Payable:</span><span className="fw-bold fs-4 text-success">₹{finalTotal}</span></div><button className="btn btn-danger w-100 fw-bold mt-4" onClick={handleSave}>{billId?'UPDATE':'SAVE BILL'}</button></div></div>
                        </div>
                    </div>
                </div>
            </div>
